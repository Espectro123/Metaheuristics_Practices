...
bool bandera = true;
int contador_evaulaciones = tamanio_poblacion;
...
for(int i = 0; i < numero_de_iteraciones && bandera; i++){
	...
	for(int j = 0; j < numero_de_cruces; j++){
		...
		unsigned short* adn;
		adn = poblacion[j].cruce_PMX(poblacion[j+1]);
	...
	//Aplico elitismo a no ser que hay un hijo mejor que el de la anterior poblacion
	for(int j = 0; j < numero_de_cruces; j++){
