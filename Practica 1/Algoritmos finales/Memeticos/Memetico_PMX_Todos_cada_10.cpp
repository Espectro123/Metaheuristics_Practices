...
unsigned int numero_de_evaulaciones = 0;
/************************************************************************************************************************************************/
...
class Individuo{
	...
	public:
		...
		//Se aplica sobre el propio objeto.
		void mutar(int gen_a_mutar,int hijo_aleatorio){

			int aleatorio = 0;
			aleatorio = Randint(0,TAM_MATRICES-1);
			unsigned short aux = 0;

				aux = this->cromosoma[gen_a_mutar];
				this->cromosoma[gen_a_mutar] = this->cromosoma[aleatorio];
				this->cromosoma[aleatorio] = aux;
				evaluado = false;
...
//Funcion de intercambio. Cambio j por k y viceversa
...
int main(int argc, char** argv){
	...
	if(argc < 8){
		...
		cout << "Maximo de iteraciones de la busqueda local" << endl;
		exit(-1);
	...
	//Obtengo las variables para el problema
	...
	//Guardara el mejor de cada poblacion. Eletismo
	Individuo Mejor_acutal;

	//Creo una poblacion de individuos
	...
	for(int i = 0; i < tamanio_poblacion; i++){
		...
		Mejor_acutal.setInicio(cromosoma,TAM_MATRICES);
	...
	for(int i = 0; i < numero_de_iteraciones && bandera; i++){
		...
		if(contador_bl == 10){
			...
			for(int j = 0; j < tamanio_poblacion-1; j++){

				busqueda_local(poblacion[j],max_iteraciones_local);
		...
		for(int j = 0; j < numero_de_cruces; j++){
			...
			unsigned short* adn;
			adn = poblacion[contador].cruce_PMX(poblacion[contador+1]);
		...
		for(int a = 0; a < TAM_MATRICES && !repetidos; a++){


			for(int b = 0; b < TAM_MATRICES && !repetidos; b++){

				if(hijos[0].getPermutacion(a) == hijos[0].getPermutacion(b) && a != b){
					repetidos = true;
				}
				if(repetidos){
					for(int c = 0; c < TAM_MATRICES; c++){
						cout << hijos[0].getPermutacion(c) << " ";
					}
					cout << endl;
				}
			}
		...
		for(int j = 0; j < numero_de_cruces; j++){
			...
			hijo_aleatorio = Randint(0,numero_de_cruces-1);
		...
		if(numero_de_evaulaciones == 50000){
			bandera = false;
		}

		ordenar_poblacion(poblacion,tamanio_poblacion);

		if(poblacion[0].getValor() > Mejor_acutal.getValor()){
			poblacion[0] = Mejor_acutal;
		}
