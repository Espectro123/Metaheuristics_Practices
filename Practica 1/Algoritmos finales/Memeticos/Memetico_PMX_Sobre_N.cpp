...
class Individuo{
	...
	public:
		...
		void mutar(int gen_a_mutar,int hijo_aleatorio){
			...
			int aleatorio = hijo_aleatorio%TAM_MATRICES;
			unsigned short aux;
...
int main(int argc, char** argv){
	...
	int numero_de_busquedas_locales = 0.1*tamanio_poblacion;
	...
	if(numero_de_busquedas_locales >= tamanio_poblacion){

		cout << "Error. Demasiadas busquedas locales para uan poblacion tan pequeña." << endl;
		exit(-1);
	...
	for(int i = 0; i < numero_de_iteraciones && bandera; i++){
		...
		for(int j = 0; j < numero_de_cruces; j++){
			...
			hijo_aleatorio = (gen_aleatorio+j)%numero_de_cruces;
